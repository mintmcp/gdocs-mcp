...
export const NOT_CONNECTED_MESSAGE =
  "Google Docs is not connected. Authorize the Google Docs connector for this user, then retry. " +
...
export const requireAccessToken: RequestHandler = (req, res, next) => {
  ...
  if (!accessToken && requiresAccessToken(req.body)) {
    res
      ...
      .set(
        "WWW-Authenticate",
        'Bearer realm="gdocs-mcp", error="invalid_token", error_description="Missing Google access token"',
      )
